...
<div class="wrapper">
  ...
  <nav class="main-header navbar navbar-expand navbar-white navbar-light">
    ...
    <ul class="nav-item d-none d-sm-inline-block">
        <li class="nav-item d-none d-sm-inline-block">
            <a href="{{ route('usuario.dashboard') }}">Seleccion de Materias</a>
        </li>
    </ul>
              <?php
                  include($_SERVER['DOCUMENT_ROOT'].'../../resources/views/conexion.php');
                  $codSis = $_GET["codigoSis"];//Auth::user()->cod_sis;
                   $query0=pg_query($conexion,"SELECT nombre FROM usuarios WHERE usuarios.cod_sis = '".$codSis."';");
                   $datos0 = pg_fetch_array($query0)                              
              ?>
    <ul class="nav-item d-none d-sm-inline-block">
        <li class="nav-item d-none d-sm-inline-block">
            <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">
                <?php echo $datos0['nombre']?>
            </a>
    ...
    <div class="sidebar">
      ...
      ?>
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <?php while($datos = pg_fetch_array($query)){ ?>
               <!--with font-awesome or any other icon font library -->
               <form action="{{ url('post') }}" method="GET">
               <input type="hidden" id="token" name="_token" value="<?php echo $token?>">
               <input type="hidden" id="codigoSis" name="codigoSis" value="<?php echo $codSis ?>">
                <li class="nav-item has-treeview">
                  <button class="btn btn-dark nav-link" type="submit">
                    <i class="nav-icon fas fa-copy"></i>
                        <p><?php echo $datos['nombre']?></p>
                  </button>
                </li>
                <input type="hidden" id="materia" name="materia" value="<?php echo $materia = $datos['nombre']?>">
                <input type="hidden" id="cargo" name="cargo" value="<?php echo $cargo = $datos['cargo']?>">
              </form>
          <?php
              }    
          ?>
